...
<head>
    ...
    {{-- Tailwind --}}
    @vite(['resources/css/app.css', 'resources/js/app.js', 'resources/js/datepicker.js', 'resources/js/swiper.js'])
        ...
        <ul class="space-y-2 font-medium">
            ...
            <li>
                <a href="{{ route('super-si.notification.index') }}" class="relative flex items-center p-2 rounded-lg {{ $pageActive == "super-si.notification" ? 'bg-slate-300 text-slate-800 ' : 'hover:bg-slate-100 group text-white hover:text-base-content' }} active:scale-95 transition-all">
                    ...
                    <span id="notif-count" class="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 bg-red-600 text-white text-xs font-bold px-1.5 py-0.5 rounded-full hidden">
                        0
                    </span>
                </a>
            </li>
...
<script type="module">
    function fetchNotificationCount() {
        fetch('/super-si/notification/unread-count')
    ...
    window.Echo.channel('inform-si').listen('InformSI', event => {
        console.log("[InformSI Event]", event);
        fetchNotificationCount();
        showPopupNotification(event.rallyGame.name);
    });

    function showPopupNotification(gameName) {
        const container = document.getElementById('popup-container');

        const wrapper = document.createElement('div');
        wrapper.className = `
            bg-yellow-400 text-slate-900 border-l-4 border-yellow-600
            p-4 rounded shadow-lg flex justify-between items-start gap-4
        ...
        wrapper.innerHTML = `
            <div class="flex-1">
                <strong class="block font-semibold mb-1">SI has been called!</strong>
                <p class="text-sm">From: <span class="font-medium">${gameName}</span></p>
            </div>
            <button onclick="this.parentElement.remove()" class="text-xl font-bold leading-none px-2 hover:text-red-600">&times;</button>
        `;
