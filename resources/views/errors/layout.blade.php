...
<head>
    ...
    <style>
        ...
        .action:hover {
            background-color: #733B22; !important;
        }
    </style>
</head>
<body>
    <div class="p-10 min-h-screen flex flex-col items-center justify-center border border-black gap-y-3 lg:gap-y-1">
        <img src="{{ asset('asset2025') }}/logo-maniac-xiv.png" alt="" class="sm:w-2/3 md:w-1/2 xl:w-1/3 select-none" draggable="false">
        <div class="flex gap-x-2 lg:gap-x-3 text-xl md:text-2xl font-black text-primary select-none">
            <p>{{ $code }}</p>
            <p>{{ $title }}</p>
        </div>
        <a href="{{ route('index') }}" class="btn btn-secondary px-5 lg:px-6 rounded mt-1 lg:mt-3 bg-light-brown action">Back to Home</a>
    </div>
</body>
</html>
