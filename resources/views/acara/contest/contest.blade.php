...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-10 w-full max-w-7xl">
        ...
        <div class="card rounded-lg shadow-md data">
            <h1 class="text-xl text-primary-content bg-primary p-5 font-medium rounded-t-lg">🏆 {{ $contest->name ?? 'Unknown Contest' }} 🏆</h1>
            <div class="card-body bg-accent rounded-b-lg">
                <h2 class="text-xl font-medium text-white mb-3">Selamat Datang, <span class="text-warning">{{ $author ?? 'Unknown Author' }}</span></h2>
                <p class="text-slate-100 pb-3 sm:pb-0 break-words">
                    Anda dapat menambahkan peserta dan melihat submission peserta pada Contest <strong class="text-base-100">{{ $contest->name ?? 'Unknown Contest' }}</strong>.
                </p>
                        ...
                        @if(count($submissions) != 0)
                            @foreach($submissions as $submission)
                                <tr>
                                    ...
                                    <td width="22.5%" class="text-center">
                                        {{ \Illuminate\Support\Carbon::createFromFormat('Y-m-d H:i:s', $submission->created_at, 'Asia/Jakarta')->format('d F Y g:i:s A') }}
                                    </td>
                                    <td width="22.5%" class="text-center">
